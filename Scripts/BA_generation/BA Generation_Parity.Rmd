...
Reference_Win <- dplyr::select(Reference_III,SEQN,CRP:PHENO_Glucose)
Reference_Win <- lapply(Reference_Win,Winsorize,minval=NULL,maxval=NULL,probs=c(0.01,0.99),na.rm=TRUE,type=7)
Reference_Win <- as.data.frame(Reference_Win)
Reference_Win <- dplyr::select(Reference_Win,CRP:PHENO_Glucose)
Temp <- dplyr::select(Reference_III,SEQN,WAVE,Age,Gender,Pregnant,BMI)
Reference_Win <- dplyr::bind_cols(Temp,Reference_Win)
summary(Reference_Win)
skewness(Reference_Win,na.rm=TRUE)
kurtosis(Reference_Win,na.rm=TRUE)

#Winsorizing outliers in analytical dataset 
skewness(Data_continuous,na.rm=TRUE)
kurtosis(Data_continuous,na.rm=TRUE)

Analysis_Win <- dplyr::select(Data_continuous,SEQN,CRP:PHENO_Glucose)
Analysis_Win <- lapply(Analysis_Win,Winsorize,minval=NULL,maxval=NULL,probs=c(0.01,0.99),na.rm=TRUE,type=7)
Analysis_Win <- as.data.frame(Analysis_Win)
Analysis_Win <- dplyr::select(Analysis_Win,CRP:PHENO_Glucose)
Temp <- dplyr::select(Data_continuous,SEQN,WAVE,Age,Gender,Pregnant,BMI)
Analysis_Win <- dplyr::bind_cols(Temp,Analysis_Win)
summary(Analysis_Win)
skewness(Analysis_Win,na.rm=TRUE)
kurtosis(Analysis_Win,na.rm=TRUE)
```
...
```{r}
#Subsetting data
BA_NHANES_Parity <- dplyr::select(Analysis_Win,SEQN,WAVE,Age,Gender,Pregnant,BMI,Albumin,PHENO_Creatinine,PHENO_Glucose,Ln_CRP,Lymphocyte_percent,MCV,Red_Cell_Width,Alkaline_Phosphatase,White_Count)
...
subset_age_nopreg <- subset(Reference_Win, Age>=30 & Age <=75 & Pregnant==2)
...
subset_both <- dplyr::select(subset_age_nopreg,Age,Albumin,PHENO_Creatinine,PHENO_Glucose,Ln_CRP,Lymphocyte_percent,MCV,Red_Cell_Width,Alkaline_Phosphatase,White_Count)
...
BA_NHANES_Parity <- dplyr::full_join(BA_NHANES_Parity,hold_both,by="SEQN")
