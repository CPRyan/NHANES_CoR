...
```
CHANGE LOG 
2020_09_10: Changed files named "creatinine" to be named "lab"" instead. This better reflects the variables within since I am extracting more than just creatinine here. Recoded NHANES III CBC data as missing for values above and below the limits of detection first reported in NHANES 2015. 

2020_09_16: Changed KDM code to calcualte all model features using only women with complete biomarker data. Changed final export to pipe out an R file instead of the convaluted SPSS files. Changed how pregnancy variable was coded in analytical sample to be nonpregnant for women aged 60 or older and missing for other women or women with uncertain status. only include women with missing data instead of recoding those with uncertain status as nonpregnant.Changed how pregnacy variable was coded in reference sample to be nonpregnant for women aged 60 or older and missing for other women or women with uncertain status.  

2020_09_18: Added code to calculate allostatic load based on distribution of biomarkers in nonpregnant women aged 18-84 (N=5870).

2020_09_23: Added a standardization step to homeostatic dysregulation construction. Specifically, the analysis proceeds by first pulling out the reference subset (N=481), merging this with the sample of women aged 18-84 with complete biomarker data (N=5870), standarizing in this merged sample, re-extracting out the reference subset, and then proceeding to calculate HD using the standardized biomarkers. In my tests this only changes the scale of HD. The variance-covariance matrix remains the same. The correlation with age seems to improve a bit, but the results in the text did not change.Therefore I am not going to use the standardization step since I didn't in my PNE publication, but will keep the code for now. 
...
```{r, include=FALSE}
DEMO <- nhanes_load_data(c("DEMO","DEMO_B","DEMO_C","DEMO_D","DEMO_E","DEMO_F"),c("1999-2000","2001-2002","2003-2004","2005-2006","2007-2008","2009-2010"))

BMI <- nhanes_load_data(c("BMX","BMX_B","BMX_C","BMX_D","BMX_E","BMX_F"),c("1999-2000","2001-2002","2003-2004","2005-2006","2007-2008","2009-2010"))

CRP <- nhanes_load_data(c("LAB11","L11_B","L11_C","CRP_D","CRP_E","CRP_F"),c("1999-2000","2001-2002","2003-2004","2005-2006","2007-2008","2009-2010"))

lab <- nhanes_load_data(c("LAB18","L40_B","L40_C","BIOPRO_D","BIOPRO_E","BIOPRO_F"),c("1999-2000","2001-2002","2003-2004","2005-2006","2007-2008","2009-2010"))

gluc <- nhanes_load_data(c("LAB10AM","L10AM_B","L10AM_C","GLU_D","GLU_E","GLU_F"),c("1999-2000","2001-2002","2003-2004","2005-2006","2007-2008","2009-2010"))

CBC <- nhanes_load_data(c("LAB25","L25_B","L25_C","CBC_D","CBC_E","CBC_F"),c("1999-2000","2001-2002","2003-2004","2005-2006","2007-2008","2009-2010"))
```
...
#Recoding pregnancy variable to be nonpregnant for all women aged 60-84 since they were not asked this question and coded as missing 
#Demographics_ALL$Pregnant[Demographics_ALL$Pregnant==3] <- 2
Demographics_ALL <- dplyr::mutate(Demographics_ALL,Pregnant = case_when(Pregnant==3 ~3, Gender==2 & Age>59 & Age<=84~2, Pregnant==1~1,Pregnant==2~2))
...
lab_1999 <- dplyr::select(lab$LAB18,SEQN,LBXSCR,LBDSALSI,LBXSAPSI)
#Correcting 1999 creatinine using the recommended equation 
lab_1999 <- dplyr::mutate(lab_1999,SEQN=SEQN, LBXSCR=(1.013*LBXSCR+0.147),LBDSALSI=LBDSALSI,LBXSAPSI=LBXSAPSI)

lab_2001 <- dplyr::select(lab$L40_B,SEQN,LBDSCR,LBDSALSI,LBDSAPSI)
colnames(lab_2001) <- colnames(lab_1999)
lab_2003 <- dplyr::select(lab$L40_C,SEQN,LBXSCR,LBDSALSI,LBXSAPSI)
lab_2005 <- dplyr::select(lab$BIOPRO_D,SEQN,LBXSCR,LBDSALSI,LBXSAPSI)

#Correcting 2005 creatinine using the recommended equation.  
lab_2005 <- dplyr::mutate(lab_2005,SEQN=SEQN,LBXSCR=(-0.016+0.978*LBXSCR),LBDSALSI=LBDSALSI,LBXSAPSI=LBXSAPSI)

lab_2007 <- dplyr::select(lab$BIOPRO_E,SEQN,LBXSCR,LBDSALSI,LBXSAPSI)
lab_2009 <- dplyr::select(lab$BIOPRO_F,SEQN,LBXSCR,LBDSALSI,LBXSAPSI)

lab_ALL <- dplyr::bind_rows(lab_1999,lab_2001,lab_2003,lab_2005,lab_2007,lab_2009)

#Creating informative names 
colnames(lab_ALL) <- c("SEQN","Creatinine","Albumin","Alkaline_Phosphatase")
```
...
Step3 <- dplyr::full_join(Step2,lab_ALL,by="SEQN")
...
```{r}
adult <- read.csv(file = "/Users/wjh180/Box/Working Files/NHANES III Datafiles/adult.csv" ,header=TRUE,sep=",")
exam <- read.csv(file = "/Users/wjh180/Box/Working Files/NHANES III Datafiles/exam.csv",header=TRUE,sep=",")
lab <- read.csv(file = "/Users/wjh180/Box/Working Files/NHANES III Datafiles/lab.csv",header=TRUE,sep=",")
```
...
#Recoding pregnancy variable to be missing for those with blank data or did not know.   
BMI_III$Pregnant[BMI_III$Pregnant==8] <- NA
BMI_III$Pregnant[BMI_III$Pregnant==9] <- NA
...
#Recoding pregnancy variable to be nonpregnant for all women aged 60-90 since they were not asked this question and coded as missing 
Step1_III <- dplyr::mutate(Step1_III,Pregnant = case_when(Gender==2 & Age>59 & Age<=90~2, Pregnant==1~1,Pregnant==2~2))
```
...
#Recoding missing and lower detection limit indicators to missing. These limits are based on the reported limits in NHANES 2015. 

Bio_cbc$LMPPCNT[Bio_cbc$LMPPCNT==88888] <- NA
Bio_cbc$WCPSI[Bio_cbc$WCPSI==88888] <- NA
Bio_cbc$WCPSI[Bio_cbc$WCPSI<=0.050] <- NA
Bio_cbc$WCPSI[Bio_cbc$WCPSI>=400] <- NA
Bio_cbc$MVPSI[Bio_cbc$MVPSI==88888] <- NA
Bio_cbc$MVPSI[Bio_cbc$MVPSI<=50.00] <- NA
Bio_cbc$MVPSI[Bio_cbc$MVPSI>=150.00] <- NA
Bio_cbc$RWP[Bio_cbc$RWP==88888] <- NA
Bio_cbc$RWP[Bio_cbc$RWP<=10.00] <- NA
Bio_cbc$RWP[Bio_cbc$RWP>=40.00] <- NA
...
#Subsetting data
BA_NHANES_Parity <- subset(BA_NHANES_Parity,Pregnant==2 & Age<=84 & Age>=18)
...
**Combining Reference Sample with Analytical Sample and Standardizing**
...
#Reference_HD_women <- dplyr::mutate(Reference_HD_women,SAMPLE=Albumin*0)

#Analytical_HD_women <- dplyr::select(BA_NHANES_Parity,Albumin,PHENO_Creatinine,PHENO_Glucose,Ln_CRP,Lymphocyte_percent,MCV,Red_Cell_Width,Alkaline_Phosphatase,White_Count)
#Analytical_HD_women <- dplyr::mutate(Analytical_HD_women,SAMPLE=Albumin*0+1)

#Combo_women <- dplyr::bind_rows(Reference_HD_women,Analytical_HD_women)
#Combo_women$SAMPLE <- as.numeric(Combo_women$SAMPLE)

#Combo_women_standard <- as.data.frame(scale(Combo_women))
```
...
```{r}
#Reference_HD_women <- subset(Combo_women_standard,SAMPLE<0)
Reference_HD_women <- dplyr::select(Reference_HD_women,Albumin,PHENO_Creatinine,PHENO_Glucose,Ln_CRP,Lymphocyte_percent,MCV,Red_Cell_Width,Alkaline_Phosphatase,White_Count)
...
```{r}
#BA_HD_Calc_women <- subset(Combo_women_standard,SAMPLE>0)
#BA_HD_Calc_women$SEQN <- BA_NHANES_Parity$SEQN
...
test <- subset(BA_NHANES_Parity, Age>=18)
cor(test$LOG_HD,test$Age,use="pairwise.complete.obs")
```
...
for(i in 2:ncol(subset2_women)) {
  correlations_women[[i]] <- cor(subset2_women[,1],subset2_women[,i],use="pairwise.complete.obs")
}
...
hold <- list()
for(i in 2:ncol(subset2_women)) {
  hold[[i]] <- lm(subset2_women[,i]~subset2_women$Age)
...
BAe_women <- list()
for(i in 1:nrow(subset2_women)) {
  BAe_women[[i]] <- sum((subset2_women[i,j+1]-parameters_women[j,1])*((parameters_women[j,2])/(parameters_women[j,3]^2)))/sum(((parameters_women[j,2])/(parameters_women[j,3]))^2)
}
...
test <- subset(BA_NHANES_Parity, Age>=18)
cor(test$KDM,test$Age,use="pairwise.complete.obs")
```

#Exporting Final Dataset

```{r}
save(BA_NHANES_Parity, file="/Users/wjh180/Documents/GitHub/NHANES_CoR/Data/NHANES_BA_Talia.rda")
```

#Allostatic Load 
Will calcualte allostatic load scores based upon the biomarker distribution within all nonpregnant women aged 18-84 with complete biomarker data.  

**Generating subset of women aged 18-84 with biomarker data**
```{r}
load("~/Documents/GitHub/NHANES_CoR/Data/NHANES_BA_Talia.rda")
allostatic <- subset(BA_NHANES_Parity,Age>17 & Age <= 84)
#NHANES <- read.csv("~/Documents/GitHub/NHANES_CoR/Data/NHANES_051720.csv", header=TRUE)
#allostatic <- dplyr::left_join(BA_NHANES_Parity,NHANES,by="SEQN")
#allostatic <- subset(allostatic,Age>17 & Age<=84 & livebirths >-1 & livebirths <=7)
```

**Establishing high risk quartiles**

```{r}
Albumin_risk <- quantile(allostatic$Albumin, probs=c(0.25),na.rm=TRUE) 
Creatinine_risk <- quantile(allostatic$PHENO_Creatinine,probs=c(0.75),na.rm=TRUE)
Glucose_risk <- quantile(allostatic$PHENO_Glucose,probs=c(0.75),na.rm=TRUE)
CRP_risk <- quantile(allostatic$Ln_CRP,probs=c(0.75),na.rm=TRUE)
Lymphocyte_risk <- quantile(allostatic$Lymphocyte_percent,probs=c(0.75),na.rm=TRUE)
MCV_risk <- quantile(allostatic$MCV,probs=c(0.75),na.rm=TRUE)
RDW_risk <- quantile(allostatic$Red_Cell_Width,probs=c(0.75),na.rm=TRUE)
Alkaline_risk <- quantile(allostatic$Alkaline_Phosphatase,probs=c(0.75),na.rm=TRUE)
WBC_risk <- quantile(allostatic$White_Count,probs=c(0.75),na.rm=TRUE)
```

**Scoring risk by biomarker**

```{r}
allostatic <- dplyr::mutate(allostatic, AL_albumin= case_when(Albumin>Albumin_risk[1]~ 0, 
                                                 Albumin<=Albumin_risk[1]~1))
allostatic <- dplyr::mutate(allostatic, AL_creatinine= case_when(PHENO_Creatinine<Creatinine_risk[1]~0,
                                                    PHENO_Creatinine>=Creatinine_risk[1]~1))
allostatic <- dplyr::mutate(allostatic, AL_glucose= case_when(PHENO_Glucose<Glucose_risk[1]~0, 
                                                PHENO_Glucose>=Glucose_risk[1]~1))
allostatic <- dplyr::mutate(allostatic, AL_CRP= case_when(Ln_CRP<CRP_risk[1]~0, 
                                            Ln_CRP>=CRP_risk[1]~1))
allostatic <- dplyr::mutate(allostatic, AL_lymphocyte= case_when(Lymphocyte_percent<Lymphocyte_risk[1]~0,
                                                   Lymphocyte_percent>=Lymphocyte_risk[1]~1))
allostatic <- dplyr::mutate(allostatic, AL_MCV= case_when(MCV<MCV_risk[1]~0,
                                             MCV>=MCV_risk[1]~1))
allostatic <- dplyr::mutate(allostatic, AL_RDW= case_when(Red_Cell_Width<RDW_risk[1]~0,
                                            Red_Cell_Width>=RDW_risk[1]~1))
allostatic <- dplyr::mutate(allostatic, AL_alkaline= case_when(Alkaline_Phosphatase<Alkaline_risk[1]~0,
                                                 Alkaline_Phosphatase>=Alkaline_risk[1]~1)) 
allostatic <- dplyr::mutate(allostatic, AL_WBC= case_when(White_Count<WBC_risk[1]~0,
                                            White_Count>=WBC_risk[1]~1))
```
...
```{r}
library(sjstats)
allostatic <- dplyr::mutate(allostatic,AL=(mean_n(allostatic[,22:30],9,digits=4)))
```

**Alternative allostatic load with two tailed risk for plausible biomarkers**

```{r}
alt <- subset(BA_NHANES_Parity,Age>17 & Age <=84)

Albumin_risk <- quantile(alt$Albumin, probs=c(0.25),na.rm=TRUE) 
Creatinine_risk <- quantile(alt$PHENO_Creatinine,probs=c(0.125,0.875),na.rm=TRUE)
Glucose_risk <- quantile(alt$PHENO_Glucose,probs=c(0.75),na.rm=TRUE)
CRP_risk <- quantile(alt$Ln_CRP,probs=c(0.75),na.rm=TRUE)
Lymphocyte_risk <- quantile(alt$Lymphocyte_percent,probs=c(0.125,0.875),na.rm=TRUE)
MCV_risk <- quantile(alt$MCV,probs=c(0.125,0.875),na.rm=TRUE)
RDW_risk <- quantile(alt$Red_Cell_Width,probs=c(0.75),na.rm=TRUE)
Alkaline_risk <- quantile(alt$Alkaline_Phosphatase,probs=c(0.75),na.rm=TRUE)
WBC_risk <- quantile(alt$White_Count,probs=c(0.125,0.875),na.rm=TRUE)

alt <- dplyr::mutate(alt, AL_albumin= case_when(Albumin>Albumin_risk[1]~ 0, 
                                                Albumin<=Albumin_risk[1]~1))
alt <- dplyr::mutate(alt, AL_creatinine= case_when(PHENO_Creatinine>Creatinine_risk[1] & PHENO_Creatinine<Creatinine_risk[2]~0,
                                                           PHENO_Creatinine<=Creatinine_risk[1] | PHENO_Creatinine>=Creatinine_risk[2]~1))
alt <- dplyr::mutate(alt, AL_glucose= case_when(PHENO_Glucose<Glucose_risk[1]~0,
                                                PHENO_Glucose>=Glucose_risk[1]~1))
alt <- dplyr::mutate(alt, AL_CRP= case_when(Ln_CRP<CRP_risk[1]~0,
                                            Ln_CRP>=CRP_risk[1]~1))
alt <- dplyr::mutate(alt, AL_lymphocyte= case_when(Lymphocyte_percent>Lymphocyte_risk[1] & Lymphocyte_percent<Lymphocyte_risk[2]~0,
                                                   Lymphocyte_percent<=Lymphocyte_risk[1] | Lymphocyte_percent>=Lymphocyte_risk[2]~1))
alt <- dplyr::mutate(alt, AL_MCV= case_when(MCV>MCV_risk[1] & MCV<MCV_risk[2]~0,
                                            MCV<=MCV_risk[1] | MCV>=MCV_risk[2]~1))
alt <- dplyr::mutate(alt, AL_RDW= case_when(Red_Cell_Width<RDW_risk[1]~0,
                                                    Red_Cell_Width>=RDW_risk[1]~1))
alt <- dplyr::mutate(alt, AL_alkaline= case_when(Alkaline_Phosphatase<Alkaline_risk[1]~0,
                                                               Alkaline_Phosphatase>=Alkaline_risk[1]~1)) 
alt <- dplyr::mutate(alt, AL_WBC= case_when(White_Count>WBC_risk[1] & White_Count<WBC_risk[2]~0,
                                                           White_Count<=WBC_risk[1] | White_Count>=WBC_risk[2]~1))

alt <- dplyr::mutate(alt,AL_alt=(mean_n(alt[,22:30],9,digits=4)))
allostatic$AL_alt <- alt$AL_alt
```

**Adding both versions of allostatic laod back to BA dataframe and exporting**

```{r}
allostatic <- dplyr::select(allostatic,SEQN,AL,AL_alt)
BA_NHANES_Parity <- dplyr::left_join(BA_NHANES_Parity,allostatic,by="SEQN")
save(BA_NHANES_Parity, file="/Users/wjh180/Documents/GitHub/NHANES_CoR/Data/NHANES_BA_Talia.rda")
```
