...
creatinine_2011 <- dplyr::select(creatinine$BIOPRO_G,SEQN,LBXSCR,LBDSALSI,LBXSAPSI)
creatinine_2013 <- dplyr::select(creatinine$BIOPRO_H,SEQN,LBXSCR,LBDSALSI,LBXSAPSI)
creatinine_2015 <- dplyr::select(creatinine$BIOPRO_I,SEQN,LBXSCR,LBDSALSI,LBXSAPSI)
...
#Combining pre-2011 measures 
gluc_late <- dplyr::bind_rows(gluc_pre2007,gluc_2007,gluc_2009)

gluc_2011 <- dplyr::select(gluc$GLU_G,SEQN,LBXGLU)
...
```{r}
#Biomarkers extracted are CRP in mg/dL (CRP), creatinine in mg/dL (CEP), Albumin in g/L (AMPSI), Alkaline Phosphatase in U/L (APPSI), & glucose in mg/dL (G1P)
...
ALL_DATA$PHENO_Glucose <- ALL_DATA$glucose*0.05551
```
...
ALL_DATA_Win <- dplyr::select(ALL_DATA,SEQN,CRP:PHENO_Glucose)
ALL_DATA_Win <- lapply(ALL_DATA_Win,Winsorize,minval=NULL,maxval=NULL,probs=c(0.01,0.99),na.rm=TRUE,type=7)
ALL_DATA_Win <- as.data.frame(ALL_DATA_Win)
ALL_DATA_Win <- dplyr::select(ALL_DATA_Win,CRP:PHENO_Glucose)
...
```{r}
#Subsetting data
BA_NHANES_Parity <- dplyr::select(ALL_DATA_Win,SEQN,WAVE,Age,Gender,Pregnant,BMI,Albumin,PHENO_Creatinine,PHENO_Glucose,Ln_CRP,Lymphocyte_percent,MCV,Red_Cell_Width,Alkaline_Phosphatase,White_Count)
BA_NHANES_Parity <- subset(BA_NHANES_Parity,Pregnant==2)
...
subset_age_nopreg <- subset(ALL_DATA_Win, WAVE==10 & Age>=30 & Age <=75 & Pregnant==2)
...
```{r}
library(foreign)
write.foreign(as.data.frame(BA_NHANES_Parity),"/Volumes/LAB COWBOY/BA and Parity/Developing Aging Measures/BA_NHANES_Parity.txt","/Volumes/LAB COWBOY/BA and Parity/Developing Aging Measures/BA_NHANES_Parity.sps", package="SPSS")
```
