...
#Creating log-transformed creatinine variable. Based on what I saw with Dan's values, this was an ln(x+1) transformation
creatinine_ALL <- dplyr::transmute(creatinine_ALL,SEQN=SEQN,Creatinine=LBXSCR,LOG_Creatinine=log(LBXSCR+1))
dim(creatinine_ALL)
...
```{r}
#Extracting albumin (LBXSAL), alkaline phosphatase (LBXSAPSI), blood urea nitrogen (LBDSBUSI), and uric acid (LBXSUA) from standard biochemistry profile dataframe named "creatinine"
lab_1999 <- dplyr::select(creatinine$LAB18,SEQN,LBXSAL,LBXSAPSI,LBDSBUSI,LBXSUA)
lab_2001 <- dplyr::select(creatinine$L40_B,SEQN,LBXSAL,LBDSAPSI,LBDSBUSI,LBXSUA)
colnames(lab_2001) <- colnames(lab_1999)
lab_2003 <- dplyr::select(creatinine$L40_C,SEQN,LBXSAL,LBXSAPSI,LBDSBUSI,LBXSUA)
lab_2005 <- dplyr::select(creatinine$BIOPRO_D,SEQN,LBXSAL,LBXSAPSI,LBDSBUSI,LBXSUA)
lab_2007 <- dplyr::select(creatinine$BIOPRO_E,SEQN,LBXSAL,LBXSAPSI,LBDSBUSI,LBXSUA)
lab_2009 <- dplyr::select(creatinine$BIOPRO_F,SEQN,LBXSAL,LBXSAPSI,LBDSBUSI,LBXSUA)
lab_2011 <- dplyr::select(creatinine$BIOPRO_G,SEQN,LBXSAL,LBXSAPSI,LBDSBUSI,LBXSUA)
lab_2013 <- dplyr::select(creatinine$BIOPRO_H,SEQN,LBXSAL,LBXSAPSI,LBDSBUSI,LBXSUA)
lab_2015 <- dplyr::select(creatinine$BIOPRO_I,SEQN,LBXSAL,LBXSAPSI,LBDSBUSI,LBXSUA)
lab_ALL <- dplyr::bind_rows(lab_1999,lab_2001,lab_2003,lab_2005,lab_2007,lab_2009,lab_2011,lab_2013,lab_2015)

#Creating informative column names and log-transformed versions of uric acid (ln(x+1)) and alkaline phosphatase
lab_ALL <- dplyr::transmute(lab_ALL,SEQN=SEQN,Albumin=LBXSAL,alkaline_phosphatase=LBXSAPSI,LOG_alkaline=log(LBXSAPSI),BUN=LBDSBUSI,uric_acid=LBXSUA,LOG_uric=log(LBXSUA+1))
...
#Adding informative column names and creating log transformed White Count. This seems to be a ln(x+1) transformation. 
cbc_ALL <- dplyr::transmute(cbc_ALL,SEQN=SEQN,Lymphocyte_percent=LBXLYPCT,White_Count=(LBXWBCSI+.14),LOG_WhiteCount=log(LBXWBCSI+1.14),MCV=LBXMCVSI,Red_Cell_Width=LBXRDW)
```
...
Bio_III$CRP[Bio_III$CRP<0.01] <- NA
...
Bio_III <- dplyr::transmute(Bio_III,SEQN=SEQN,CRP=CRP,LOG_CRP=log(CRP),HBA1c=HBA1c,Creatinine=(Creatinine*0.960-0.184),LOG_Creatinine=log((Creatinine*0.960-0.184)+1),glucose=glucose*0.05551)

#Extracting comparison panel biomarkers
Bio_compare <- dplyr::select(lab,SEQN,AMP,APPSI,BUPSI,UAP,LMPPCNT,WCPSI,MVPSI,RWP)
...
Bio_compare$BUP[Bio_compare$BUPSI==88888] <- NA
...
Bio_compare <- dplyr::transmute(Bio_compare,SEQN=SEQN,Albumin=AMP,alkaline_phosphatase=APPSI,LOG_alkaline=log(APPSI),BUN=BUPSI,uric_acid=UAP,LOG_uric=log(UAP),Lymphocyte_percent=LMPPCNT,White_Count=WCPSI+0.14,LOG_WhiteCount=log(WCPSI+1.14),MCV=MVPSI,Red_Cell_Width=RWP)
```
...
#Dan seems to have removed some outlier values for certain biomarkers. I've made my best guess as to what these cutoffs would be below. 
ALL_DATA$CRP[ALL_DATA$CRP>10] <- NA
ALL_DATA$HBA1c[ALL_DATA$HBA1c>12] <- NA 
ALL_DATA$Albumin[ALL_DATA$Albumin<2.5] <- NA
ALL_DATA$Albumin[ALL_DATA$Albumin>5.5] <- NA
ALL_DATA$LOG_alkaline[ALL_DATA$LOG_alkaline<2.75] <- NA
ALL_DATA$LOG_alkaline[ALL_DATA$LOG_alkaline>6.0] <- NA
ALL_DATA$LOG_uric[ALL_DATA$LOG_uric<0] <- NA 
ALL_DATA$Lymphocyte_percent[ALL_DATA$Lymphocyte_percent>75] <-NA 
ALL_DATA$LOG_WhiteCount[ALL_DATA$LOG_WhiteCount>3.5] <- NA
ALL_DATA$MCV[ALL_DATA$MCV<59] <- NA 
ALL_DATA$MCV[ALL_DATA$MCV>117.5] <- NA 
ALL_DATA$average_systolic[ALL_DATA$average_systolic>225] <- NA

#Creating versions of Creatinine and Albumin needed for PhenoAge algorithm 
ALL_DATA$PHENO_Creatinine <- ALL_DATA$Creatinine*88.42
ALL_DATA$PHENO_Albumin <- ALL_DATA$Albumin*10


#Creating subset of data to compare biomarker values to PNE paper values 
new <- subset(Data_continuous, WAVE<=2)
summary(new)
```
...
There are some differences here but I think it is because I used published equations to adjust creatinine values. 


```{r}
#Subsetting data
BA_NHANES_Parity <- dplyr::select(ALL_DATA,SEQN,WAVE,Age,Gender,Pregnant,PHENO_Albumin,PHENO_Creatinine,glucose,LOG_CRP,Lymphocyte_percent,MCV,Red_Cell_Width,alkaline_phosphatase,White_Count)

BA_NHANES_Parity <- subset(BA_NHANES_Parity, WAVE<=2  & Age<=84 & Pregnant==2)
...
gamma <- 0.0076927
BA_NHANES_Parity <- dplyr::mutate(BA_NHANES_Parity,CDF=1-(exp(-1.51714*exp(xb)/gamma)))
...
**Subsetting data**
Need to check on cut-off values for red  cell width. Need to change cut-off for BUN to mmol/L instead of mg/dL

```{r}
#Reference dataframe only includes women for the sake of this project 
Reference_HD_women <- subset(ALL_DATA,WAVE>=3 & Gender==2 & Age>=20 & Age<=30 & BMI<30 & Pregnant==2 & Albumin>3.5 & Albumin<5.0 & alkaline_phosphatase>37 & alkaline_phosphatase<98 & BUN>=6 & BUN<21 & Creatinine>0.6 & Creatinine<1.1 & CRP<=2 & HBA1c>4.0 & HBA1c<5.6 & uric_acid>2.7 & uric_acid<=6.1 & Lymphocyte_percent>28 & Lymphocyte_percent<=55 & MCV>77 & MCV<93 & Red_Cell_Width>11.5 & Red_Cell_Width<14.5 & White_Count<=11 & average_systolic<120)

Reference_HD_men <- subset(ALL_DATA,WAVE>=3 & Gender==1 & Age>=20 & Age<=30 & BMI<30 & Pregnant==2 & Albumin>3.5 & Albumin<5.0 & alkaline_phosphatase>45 & alkaline_phosphatase<115 & BUN>=8 & BUN<24 & Creatinine>0.8 & Creatinine<1.3 & CRP<2 & HBA1c>=4.0 & HBA1c<5.6 & uric_acid>3.7 & uric_acid<=8.0 & Lymphocyte_percent>28 & Lymphocyte_percent<=55 & MCV>77 & MCV<93 & Red_Cell_Width>11.5 & Red_Cell_Width<14.5 & White_Count<=11 & average_systolic<120)
...
```{r}
Reference_HD_women <- dplyr::select(Reference_HD_women,Albumin,LOG_alkaline,BUN,LOG_Creatinine,LOG_CRP,HBA1c,LOG_uric,Lymphocyte_percent,LOG_WhiteCount,MCV,Red_Cell_Width,average_systolic)

Reference_HD_men <- dplyr::select(Reference_HD_men,Albumin,LOG_alkaline,BUN,LOG_Creatinine,LOG_CRP,HBA1c,LOG_uric,Lymphocyte_percent,LOG_WhiteCount,MCV,Red_Cell_Width,average_systolic)
...
BA_HD_Calc_women <- subset(ALL_DATA,Gender==2 & WAVE<=2 & Age>=18 & Age<=84 & Pregnant==2)
BA_HD_Calc_men <- subset(ALL_DATA,Gender==1 & WAVE<=2 & Age>=18 & Age<=84)

BA_HD_Calc2_women <- dplyr::select(BA_HD_Calc_women,Albumin,LOG_alkaline,BUN,LOG_Creatinine,LOG_CRP,HBA1c,LOG_uric,Lymphocyte_percent,LOG_WhiteCount,MCV,Red_Cell_Width,average_systolic)


BA_HD_Calc2_men <- dplyr::select(BA_HD_Calc_men,Albumin,LOG_alkaline,BUN,LOG_Creatinine,LOG_CRP,HBA1c,LOG_uric,Lymphocyte_percent,LOG_WhiteCount,MCV,Red_Cell_Width,average_systolic)
...
subset_age_nopreg <- subset(ALL_DATA, WAVE>2 & Age>=30 & Age <=75 & Pregnant==2)
...
subset_both <- dplyr::select(subset_age_nopreg,Age,Albumin,LOG_alkaline,BUN,LOG_Creatinine,LOG_CRP,HBA1c,LOG_uric,Lymphocyte_percent,LOG_WhiteCount,MCV,Red_Cell_Width,average_systolic)

subset_men <- dplyr::select(subset_men,Age,Albumin,LOG_alkaline,BUN,LOG_Creatinine,LOG_CRP,HBA1c,LOG_uric,Lymphocyte_percent,LOG_WhiteCount,MCV,Red_Cell_Width,average_systolic)

subset_women <- dplyr::select(subset_women,Age,Albumin,LOG_alkaline,BUN,LOG_Creatinine,LOG_CRP,HBA1c,LOG_uric,Lymphocyte_percent,LOG_WhiteCount,MCV,Red_Cell_Width,average_systolic)
```
...
parameters_men <- matrix(nrow=12,ncol=3)
...
parameters_women <- matrix(nrow=12,ncol=3)
...
```{r}
mydata_subset <- dplyr::select(ALL_DATA,Age,Albumin,Log_alkaline,BUN,LOG_Creatinine,HBA1c,LOG_CRP,Lymphocyte_percent,LOG_WhiteCount,MCV,Red_Cell_Width,average_systolic,SEQN,Gender)
mydata_men <- subset(mydata_subset,WAVE<=2 & Gender==1 & Pregnant==2 & Age>=18 & Age<=84)
mydata_women <- subset(mydata_subset, WAVE<=2 & Gender==2 & Pregnant==2 & Age>=18 & Age<=84)
```
