---
title: "R code for 'Parity predicts biological age acceleration but only in postmenopausal women: Evidence from NHANES 1999-2010'
"
author: "Talia Shirazi, Waylon Hastings, Asher Rosinger, and Calen Ryan"
date: "10/6/2020"
...
#Stratified Analyses: Pre-menopausal Women
...
#Stratified Analyses: Post-menopausal Women 
...
#Analyzing BMI as a categorical variable

```{r}
#Primary analysis, pre-menopausal
nhanesDesign1<- subset(base, RIDAGEYR > 17 & RIDAGEYR <=84 & RIAGENDR == 2 & livebirths > -1 & livebirths < 8 & RIDEXPRG == 2 & menopause == 0)

summ(LM1 <- svyglm(LM_age_resid2_premeno ~ livebirths + livebirths2 + RIDAGEYR + BMI_cat + INDFMPIR + smoking + DMDEDUC2 + RIDRETH1, design = nhanesDesign1, data = df), confint = TRUE, digits = 3)
summ(HD1 <- svyglm(HD_age_resid2_premeno ~ livebirths + livebirths2 + RIDAGEYR + BMI_cat + INDFMPIR + smoking + DMDEDUC2 + RIDRETH1, design = nhanesDesign1, data = df), confint = TRUE, digits = 3)
summ(KDM1 <- svyglm(KDM_age_resid2_premeno ~ livebirths + livebirths2 + RIDAGEYR + BMI_cat + INDFMPIR + smoking + DMDEDUC2 + RIDRETH1, design = nhanesDesign1, data = df), confint = TRUE, digits = 3)
summ(AL1 <- svyglm(AL_age_resid2_premeno ~ livebirths + livebirths2 + RIDAGEYR + BMI_cat + INDFMPIR + smoking + DMDEDUC2 + RIDRETH1, design = nhanesDesign1, data = df), confint = TRUE, digits = 3)

#Primary analysis, post-menopausal
nhanesDesign1<- subset(base, RIDAGEYR > 17 & RIDAGEYR <=84 & RIAGENDR == 2 & livebirths > -1 & livebirths < 8 & RIDEXPRG == 2 & menopause == 1)
